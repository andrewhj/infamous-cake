...
trait TerminalUserInterfaceModule {
  ...
  object userInterface {
    ...
    private def withDeadlinePrompt(onSuccess: LocalDateTime => Unit): Unit = {
      ...
      val userInput = StdIn.readLine(s"Please enter a deadline ($format) ").trim
      Try (LocalDateTime.parse(userInput, formatter)).toOption match {
        case Some(deadline) => onSuccess(deadline)
        case None => println(s"\n${Console.YELLOW + userInput + Console.RED} does not match required format $format${Console.RESET}")
      }
    }
  ...
  private def deleteAll(): Unit = {
    ...
    println(Console.GREEN + "deleted all todos" + Console.RESET)
  }
  ...
  private lazy val OldestFirst: Ordering[LocalDateTime] = 
    ...
    private def updateDescription(): Unit = {
      withIdPrompt { id => 
        withReadOne(id) { todo =>
          withDescriptionPrompt { desc =>
            val updatedTodo =
              todo.withUpdatedDescription(desc)
    ...
    private def updateDeadline(): Unit = {
      withIdPrompt { id =>
        withReadOne(id) { todo => 
          withDeadlinePrompt { dl =>
            val updatedTodo =
              todo.withUpdatedDeadline(dl)
            applicationBoundary.updateOne(updatedTodo)
